public class RollTwoPairs {
	...
	public static void main(String[] args) {
		...
		do {	// Roll the two pairs of dice until totals are the same.
			...
			TextIO.putln("First pair comes up " + total1);
			
			secondDice.roll();	// Roll the second pair of dice.
			total2 = secondDice.die1 + secondDice.die2; //Get total.
			TextIO.putln("Second pair comes up " + total2);
			
			countRolls++;	// Count this roll.
			
			TextIO.putln();	// Blank line.
			
		} while (total1 != total2);
		
		TextIO.putln("It took " + countRolls + " rolls until the totals were the same.");
